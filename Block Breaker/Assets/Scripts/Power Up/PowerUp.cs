        ...
        private void ApplyPowerUpEffects()
        {
            switch (_effects)
            {
                case Effects._multiBall:
                    if (!_gameManager._gameEnded) // if game is ended and we are still getting power up then do not apply effects
                    {
                        _powerUpManager.MultiBall();
                    }
                    break;
                ...
                case Effects._laser:
                    if (!_gameManager._gameEnded) // if game is ended and we are still getting power up then do not apply effects
                    {
                        ...
                        {
                            _powerUpManager.StopCoroutine(_powerUpManager.LaserShotCoroutine);
                        }
                        _powerUpManager.StartLaserShot();
                    }
                    break;
            }
        }
